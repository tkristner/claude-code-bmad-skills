...
description: Designs system architecture, selects tech stacks, defines components and interfaces, addresses non-functional requirements. Validates implementation readiness before Phase 4. Trigger words - architecture, system design, tech stack, components, scalability, security, API design, data model, NFR, patterns, microservices, monolith, implementation readiness, ready to implement, gate check, validate planning, pre-implementation review
...
5. **Document Decisions** - Every major decision has a "why" (use ADRs)
6. **Context Aware** - Adapt approach for greenfield vs brownfield

## Brownfield vs Greenfield Architecture

### Greenfield Projects (New Development)
- Full freedom in technology selection
- Design from first principles
- Focus on ideal patterns and practices
- No legacy constraints

### Brownfield Projects (Existing System)
- **Analyze current state first** before proposing changes
- Respect existing technology choices unless migration is justified
- Design for incremental improvement
- Consider backwards compatibility
- Document integration points with existing systems

**Brownfield Architecture Checklist:**
1. Map existing components and their responsibilities
2. Identify integration points and APIs
3. Document current technology stack and versions
4. Assess technical debt to address or work around
5. Plan migration path if changing patterns
6. Define backwards compatibility requirements

### Architecture Decision Records (ADRs)

For significant decisions, create ADRs in `docs/adr/` using the template:

> **Template:** `templates/adr.template.md`

The ADR format follows the Nygard/MADR industry standard with these sections:
...
**Trigger:** User requests implementation readiness check, gate check, or pre-implementation review

**Steps:**
1. Inventory all planning documents (PRD, Architecture, Epics, Stories)
2. Analyze PRD for complete Functional Requirements coverage
3. Validate Architecture addresses all NFRs
4. Create traceability matrix (FRs → Stories)
...
- For brownfield projects, understand before redesigning
- Use ADRs for decisions that are hard to reverse
- Run `/check-implementation-readiness` before handing off to Phase 4
