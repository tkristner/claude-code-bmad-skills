...
- Create UX/UI designs for new features or screens
- Design wireframes and user flows
- Ensure accessibility compliance (WCAG 2.1 AA)
- Create design systems or component specifications
...
### /create-ux-design
Create comprehensive UX design with wireframes, flows, and accessibility.

**Workflow:** See [workflows/create-ux-design.md](workflows/create-ux-design.md)

**Process:** Load requirements → Define scope → User flows → Wireframes → Accessibility → Components → Design tokens → Developer handoff

**Output:** Complete UX design document ready for implementation

### /wcag-validate
Validate design or implementation against WCAG 2.1 AA accessibility standards.

**Workflow:** See [workflows/wcag-validate.md](workflows/wcag-validate.md)

**Process:** Identify scope → Run WCAG checklist → Check contrast → Validate keyboard nav → Generate compliance report

**Output:** WCAG compliance report with pass/fail status and remediation recommendations

### /check-contrast
Quick color contrast validation for WCAG compliance.

**Workflow:** See [workflows/check-contrast.md](workflows/check-contrast.md)

**Process:** Collect color pairs → Calculate ratios → Evaluate pass/fail → Suggest alternatives

**Output:** Contrast ratio report with accessibility status per WCAG level
...
8. **Handoff-Ready** - Designs are implementable without ambiguity
...
**Handoff Checklist:**
```
[ ] All screens documented with wireframes
...
[ ] Interaction timing (animation duration if any)
[ ] Edge cases documented (empty states, error states)
```

**What Developers Need:**
- **Not** pixel-perfect mockups (AI can't produce images)
- **Yes** detailed specifications in structured format
- **Yes** clear component hierarchy
- **Yes** exact color and spacing values
- **Yes** all states enumerated
- **Yes** responsive breakpoint behavior
...
- Organisms: forms, headers, footers
...
**Without visual prototypes, test designs by:**

1. **Walkthrough Reviews**
   - Walk through user flows verbally with stakeholders
   - Describe each screen and interaction
   - Ask "What would you expect here?"

2. **Specification Review**
   - Review wireframe specifications with developers
   - Identify implementation concerns early
   - Validate technical feasibility

3. **Accessibility Audit**
   - Run WCAG checklist against specifications
   - Verify keyboard navigation paths
   - Check contrast ratios for all color combinations

4. **Edge Case Analysis**
   - Document all empty states
   - Define all error states and messages
