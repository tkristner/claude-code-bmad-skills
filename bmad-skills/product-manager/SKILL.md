...
description: Product requirements and planning specialist. Creates PRDs and tech specs with functional/non-functional requirements, prioritizes features using MoSCoW/RICE frameworks, breaks down epics into user stories, and ensures requirements are testable and traceable. Handles both greenfield and brownfield projects with continuation support. Use for PRD creation, requirements definition, feature prioritization, tech specs, epics, user stories, acceptance criteria, quick spec, quick requirements, lightweight spec, validate PRD, PRD validation, requirements quality check, brownfield requirements, existing system enhancement.
...
## PRD vs Tech Spec vs Quick-Spec Decision Logic

| Use PRD | Use Tech Spec | Use Quick-Spec |
|---------|---------------|----------------|
| Level 2+ projects | Level 0-1 projects | Small focused features |
| Multiple stakeholders | Limited stakeholders | Single developer |
| Extensive requirements | Clear scope | 1-5 story points |
| Long-term roadmap | Quick delivery | Need to move fast |
| Cross-functional | Technical-focused | Brownfield/greenfield |

**Quick-Spec Workflow:** See [workflows/quick-spec.md](workflows/quick-spec.md)
- Conversational spec engineering (15-45 min)
- Scans code before asking questions
- "Ready for Development" standard
- Produces specs for /quick-dev

**Invoke:** `/quick-spec`

**Validate PRD Workflow:** See [workflows/validate-prd.md](workflows/validate-prd.md)
- Systematic PRD quality validation
- Checks completeness, measurability, traceability
- Generates validation report with findings
- Ensures PRD is ready for architecture
...
**Continuation Steps:**
1. Load existing document completely
2. Read frontmatter to identify completed sections
3. Present status to user:
   ```
   Found existing PRD in progress.

   Completed sections: [list]
   Next section: [section name]

   [R] Resume from where we left off
   [S] Start fresh (will overwrite)
   [V] View current document
   ```
4. Resume without re-asking completed questions

**State Tracking:**
- Track progress in document frontmatter
- Update `stepsCompleted` array after each section
- Store input document references in `inputDocuments`
...
- Load and analyze existing codebase first
- Identify integration constraints
- Focus on delta (what's new/changed)
...
- Deprecated Features (if applicable)

**Discovery Questions for Brownfield:**
1. What existing functionality must be preserved?
2. What integrations exist that we must maintain?
3. Are there performance baselines we must meet or exceed?
4. What technical debt should we address in this scope?
5. What documentation exists for the current system?
...
### /prd
Create comprehensive Product Requirements Document for Level 2+ projects.

**Workflow:** See [workflows/prd.md](workflows/prd.md)

**Process:** Load product brief → Gather FRs → Gather NFRs → Create Epics → Generate document

**Output:** Complete PRD with traceable requirements, prioritization, and epic breakdown

### /tech-spec
Create focused technical specification for Level 0-1 projects.

**Workflow:** See [workflows/tech-spec.md](workflows/tech-spec.md)

**Process:** Problem/solution → Requirements list → Technical approach → Implementation plan

**Output:** Lightweight tech spec ready for quick implementation
